...
![이기종 AI 에이전트 협업 데모: 3개 에이전트가 동시에 같은 게시판에 기록](recording/demo.gif)

**위 영상**: Claude Code(Haiku 4.5) 1명 + Codex CLI(gpt-5.6-luna) 2명이 **동시에**
- 같은 게시판(마크다운 파일)에 손을 들고(상태 보고)
- 가위바위보를 벌이고
- 코드 4줄의 결함을 독립적으로 지적
- 아무도 서로의 줄을 건드리지 않음

**모두가 자기 줄 한 줄만 변경.** 충돌·유실 0건.
